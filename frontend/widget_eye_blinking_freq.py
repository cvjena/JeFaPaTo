...
class WidgetEyeBlinkingFreq(QSplitter):
    def __init__(self):
        ...
        self.settings.addRow(self.te_results)
        ...
        self._load_file(
            Path(
                (
                    "/home/buechner/data/Med. Doktoranden/Jefapato_Schuhmann/Proband"
                    "06_2021-09-24_14-56-34.csv"
                )
            )
        )
    ...
    def _load_file(self, path: Path) -> None:
        ...
        with open(path) as file:
            ...
            for row in reader:
                items = [QtGui.QStandardItem(f) for f in row]
